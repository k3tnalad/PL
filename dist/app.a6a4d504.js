...
exports.fixtureHeaderInners = fixtureHeaderInners;
const fixtureDataTabs = fixtureTabsContainer.querySelectorAll("[data-tab-target]");
exports.fixtureDataTabs = fixtureDataTabs;
...
exports.weekNavButtons = weekNavButtons;
...
exports.eventTypeHandler = eventTypeHandler;
...
const animateTabs = e => {
  if (e.target.classList.contains("fixturesBtn")) {
    [_elements.standingsSection, _elements.statsSection].forEach(elem => hideElem(elem));
    showElem(_elements.fixturesSection);
  } else if (e.target.classList.contains("standingsBtn")) {
    [_elements.fixturesSection, _elements.statsSection].forEach(elem => hideElem(elem));
    showElem(_elements.standingsSection);
  } else if (e.target.classList.contains('statsBtn')) {
    [_elements.standingsSection, _elements.fixturesSection].forEach(elem => hideElem(elem));
    showElem(_elements.statsSection);
  }
};
...
const matchPop = async e => {
  ...
  _elements.fixtureHeaderInners[1].textContent = matchBlob.score.fulltime || `n/a`;
  ...
  if (matchBlob.statusShort === "NS") {
    _elements.timelineTab.innerHTML = `
            <h3 style="text-align:center">Match ig going to be played ${(0, _moment.default)(matchBlob.event_timestamp).format('MMMM Do YYYY, h:mm')}</h3>
        `;
  } else {
    matchObj = {
    ...
    let statsHTML = `
        ...
        `;
    _elements.statsTab.innerHTML = statsHTML;
  }
};
...
_elements.weekNavButtons.forEach(btn => btn.addEventListener('click', () => _elements.fixturesList.classList.add('skipped')));

_elements.fixturesList.addEventListener('animationend', () => _elements.fixturesList.classList.remove('skipped'));

[..._elements.fixturesList.children].forEach(child => child.addEventListener('click', e => {
  matchPop(e);
}));
...
_elements.fixtureDataTabs.forEach(t => t.addEventListener('click', () => {
  ...
  _elements.tabsContent.forEach(tc => tc.classList.remove('active'));
